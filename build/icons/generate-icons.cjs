...
function hasImageMagick() {
  try {
    ...
    // Fallback to convert for older ImageMagick
    try {
...
function generatePNG(size, outputPath) {
  try {
    execSync(`magick -background none -resize ${size}x${size} "${svgPath}" "${outputPath}"`, {
      stdio: 'ignore'
    });
...
function generateICO() {
  ...
  try {
    ...
    const sizeList = winSizes.join(',');
    execSync(`magick "${png256}" -define icon:auto-resize=${sizeList} "${icoPath}"`, {
      stdio: 'ignore'
    });
